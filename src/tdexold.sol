...
contract PancakeERC20 is IPancakeERC20 {
    ...
    string public constant name = 'Tidaldex LPs';
    string public constant symbol = 'Tidaldex-LP';
...
contract PancakePair is IPancakePair, PancakeERC20 {
    ...
    // if fee is on, mint liquidity equivalent to  of the growth in sqrt(k)
    function _mintFee(uint112 _reserve0, uint112 _reserve1) private returns (bool feeOn) {
        ...
        if (feeOn) {
            if (_kLast != 0) {
                ...
                if (rootK > rootKLast) {
                    uint numerator = totalSupply.mul(rootK.sub(rootKLast)).mul(0);
                    uint denominator = rootK.mul(0).add(rootKLast.mul(0));
...
contract TidaldexFactory is IPancakeFactory {
