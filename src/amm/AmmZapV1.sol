 ...
 */
contract AmmZapV1 is Ownable, ReentrancyGuard {
    ...
    IAmmRouter02 public pancakeRouter;
    ...
    constructor(
        ...
        pancakeRouter = IAmmRouter02(_pancakeRouter);
     ...
     * @param _tokenAmountOutMin: minimum token amount (e.g. CAKE)
     * to receive in the intermediary swap (e.g. BNB --> CAKE)
     */
    function zapInBNB(
        address _lpToken,
    ...
    {
        address token0 = IAmmPair(_lpToken).token0();
        address token1 = IAmmPair(_lpToken).token1();
        ...
        (uint256 reserveA, uint256 reserveB, ) = IAmmPair(_lpToken)
            .getReserves();
    ...
    {
        require(
            _token0ToZap == IAmmPair(_lpToken).token0() ||
                _token0ToZap == IAmmPair(_lpToken).token1(),
            "Zap: Wrong token0"
        );
        require(
            _token1ToZap == IAmmPair(_lpToken).token0() ||
                _token1ToZap == IAmmPair(_lpToken).token1(),
            "Zap: Wrong token1"
        );
        ...
        if (_token0ToZap == IAmmPair(_lpToken).token0()) {
    ...
    function _zapOut(
        ...
        (uint256 amount0, uint256 amount1) = IAmmPair(_lpToken).burn(
            address(this)
        );
    ...
    function _approveTokenIfNeeded(address _token) private {
        if (
            IERC20(_token).allowance(address(this), pancakeRouterAddress) < 1e24
        ) {
            // Re-approve
            IERC20(_token).approve(pancakeRouterAddress, MAX_INT);
        }
    }
